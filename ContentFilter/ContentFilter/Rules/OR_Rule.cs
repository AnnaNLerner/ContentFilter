...
{
    public class OR_Rule : GeneralRule
    {
        public HashSet<GeneralRule> _orRules = new HashSet<GeneralRule>();
        ...
        public GeneralRule initRule(KeyValuePair<string,string> rule)
        {
            ...
            setName(rule.Key);
            if (rule.Value.ToLowerInvariant().Trim().StartsWith("or"))
            {
                ...
                if (match.Length > 0)
                {
                    string[] rules = new string[match.Length]; //IGNORE FIRST MATCH OF "or"
        ...
        public override RuleResult Activate(string sentence)
        {
            ...
            foreach (GeneralRule rx in _orRules)
            {
